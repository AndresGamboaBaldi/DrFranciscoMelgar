      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '5rem', flexWrap: 'wrap', gap: '2rem' }}>
        ...
        <Reveal delay={200}>
          <p style={{ maxWidth: '28rem', fontSize: '.88rem', lineHeight: 1.9, color: 'var(--color-ink-dim)', fontWeight: 300 }}>
            Selecciona tu tratamiento, elige fecha y hora, y completa tus datos. Confirmaremos en menos de 24 horas.
          </p>
        </Reveal>
      </div>
        ...
        <div style={{ maxWidth: '62rem', margin: '0 auto' }}>
          {success ? (
            ...
            <>
              ...
              {step === 1 && (
                <ServiceSelector selected={service} onSelect={svc => { setService(svc) }} />
              )}
              {step === 2 && (
                <CalendarPicker
                  selectedDate={date}
...
function StepNav({ step }: { step: BookingStep }) {
  const steps = [{ label: 'Tratamiento' }, { label: 'Fecha & Hora' }, { label: 'Tus Datos' }]
  return (
    <div style={{ display: 'flex', borderBottom: '1px solid var(--color-rim)', marginBottom: '3rem' }}>
      {steps.map((s, i) => {
        ...
        const done   = n < step
        return (
          <div key={n} style={{ display: 'flex', alignItems: 'center', gap: '.7rem', padding: '1rem 2rem 1rem 0', opacity: active ? 1 : done ? .55 : .38, transition: 'opacity .3s' }}>
            ...
            <span style={{ fontSize: '.72rem', letterSpacing: '.1em', textTransform: 'uppercase', color: active ? 'var(--color-ink)' : 'var(--color-ink-dim)' }}>{s.label}</span>
          </div>
        )
      })}
    </div>
...
function SuccessState({ onReset }: { onReset: () => void }) {
  return (
    <div style={{ textAlign: 'center', padding: '4.5rem 2rem' }}>
      ...
      <h3 style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '2.2rem', fontWeight: 300, marginBottom: '.75rem' }}>¡Cita Solicitada con Éxito!</h3>
      <p style={{ fontSize: '.87rem', color: 'var(--color-ink-dim)', maxWidth: '28rem', margin: '0 auto 2rem', lineHeight: 1.9 }}>
        Hemos recibido tu solicitud y guardado todos los detalles. Te contactaremos en las próximas horas para confirmar.<br /><br />
        <span style={{ color: 'var(--color-gold)' }}>¡Te esperamos muy pronto!</span>
      </p>
      <button onClick={onReset}
        style={{ display: 'inline-flex', alignItems: 'center', gap: '.7rem', padding: '.85rem 2rem', background: 'transparent', color: 'var(--color-ink-dim)', fontFamily: 'var(--font-body)', fontSize: '.72rem', fontWeight: 300, letterSpacing: '.14em', textTransform: 'uppercase', border: '1px solid var(--color-rim-l)', cursor: 'pointer', transition: 'all .3s' }}
        onMouseEnter={e => { e.currentTarget.style.color = 'var(--color-ink)'; e.currentTarget.style.borderColor = 'var(--color-ink-ghost)' }}
        onMouseLeave={e => { e.currentTarget.style.color = 'var(--color-ink-dim)'; e.currentTarget.style.borderColor = 'var(--color-rim-l)' }}
      >
