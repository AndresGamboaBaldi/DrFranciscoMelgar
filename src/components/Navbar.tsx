...
export default function Navbar() {
  ...
  return (
    <nav style={{
      ...
      <a href="#citas" className="hidden md:inline-flex"
      ...
      <button onClick={() => setOpen(!open)} className="flex md:hidden flex-col gap-[5px] cursor-pointer bg-transparent border-none p-1" aria-label="Menú">
        {[0,1,2].map(i => <span key={i} style={{ display: 'block', width: 22, height: 1, background: 'var(--color-ink-dim)' }} />)}
      </button>

      {open && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'rgba(10,9,7,.97)', backdropFilter: 'blur(14px)', borderBottom: '1px solid var(--color-rim)', padding: '1.5rem 2.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          {links.map(l => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)}
              style={{ fontSize: '.8rem', letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--color-ink-dim)', textDecoration: 'none' }}>{l.label}</a>
          ))}
          <a href="#citas" onClick={() => setOpen(false)}
            style={{ fontSize: '.8rem', letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--color-bg)', background: 'var(--color-gold)', padding: '.65rem 1.5rem', textDecoration: 'none', textAlign: 'center' }}>Reservar Cita</a>
        </div>
      )}
    </nav>
  )
